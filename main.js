document.addEventListener("DOMContentLoaded", function () {
    ...
    const cartDropdownClose = document.getElementById("cartDropdownClose");
    ...
    function renderCartDropdown() {
        ...
        if (entries.length === 0) {
            cartDropdownBody.innerHTML = `<div class="cart-empty-msg">
                <i class="fas fa-shopping-cart"></i>
                <p>ยังไม่มีรายการในตะกร้า</p>
                <span>เลือกเมนูที่ชอบแล้วกด + เพื่อเพิ่มครับ</span>
            </div>`;
        ...
        let total = 0;
        cartDropdownBody.innerHTML = entries.map(([name, qty]) => {
            const price = itemPrices[name] || 0;
            const subtotal = price * qty;
            total += subtotal;
            return `<div class="cart-drop-item">
                <span class="cart-drop-item-qty">${qty}</span>
                <span class="cart-drop-item-name">${name}</span>
                <span class="cart-drop-item-price">฿${subtotal}</span>
                <button class="cart-drop-item-remove" data-name="${name}" title="ลบ"><i class="fas fa-times"></i></button>
            </div>`;
        }).join("");

        cartDropdownTotal.textContent = "฿" + total.toLocaleString();
        cartDropdownFooter.style.display = "block";

        // Remove item buttons
        cartDropdownBody.querySelectorAll(".cart-drop-item-remove").forEach(btn => {
